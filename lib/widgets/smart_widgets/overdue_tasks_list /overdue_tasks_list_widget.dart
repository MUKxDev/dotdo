...
import 'overdue_tasks_list_view_model.dart';

class OverdueTasksListWidget extends StatelessWidget {
  const OverdueTasksListWidget({Key key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return ViewModelBuilder<OverdueTasksListViewModel>.reactive(
      // onModelReady: (OverdueTasksListViewModel viewModel) =>
      //     viewModel.handleOnStartUp(),
      builder: (BuildContext context, OverdueTasksListViewModel viewModel,
          Widget _) {
          ...
          builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
            ...
            return (snapshot.hasData == false || snapshot.data.size == 0)
                ...
                : Column(
                    ...
                    children: [
                      // * Overdue Header Test
                      HeaderTextWidget(lable: 'Overdue'),
                      ListView.builder(
                        key: UniqueKey(),
      ...
      viewModelBuilder: () => OverdueTasksListViewModel(),
    );
  }
}
